...
{    
    ...
    labInfo4.setText("v2.01", juce::dontSendNotification);
...
void MakoBiteAudioProcessorEditor::KNOB_ShowValue(int t)
{
    ...
    switch (Knob_Info[t].DataType)
    {
      ...
      case 5: //R2.00 Special formatting for Low CUT. 
          ...
          break;
      case 6: //R2.01 Show the current amplifier selection.
          switch (int(sldKnob[t].getValue()))
          {
          ...
          break;
      case 7: //R2.01 Show the current amplifier selection.
          switch (int(sldKnob[t].getValue()))
          {
          ...
          break;
      case 8: //R2.01 Show the current Boost selection.          
          switch (int(sldKnob[t].getValue()))
          {
...
{       
    ...
    {   
        //R2.01 Treat as Read Only *audioProcessor.Setting[e_EQ] = float(sldKnob[e_EQ].getValue());
    ...
    for (int t = 0; t < Knob_Cnt; t++)
    {
        ...
        {   
            ...
            //R2.01 Treat as Read Only *audioProcessor.Setting[t] = float(sldKnob[t].getValue());
